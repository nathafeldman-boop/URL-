/**
 * Verdict — audit de conversion pour landing pages.
 * Serveur HTTP sans dépendance : sert le front statique et expose POST /api/analyze.
 */
...
async function handleAnalyze(req, res) {
  if (!MISTRAL_API_KEY) {
    return sendJson(res, 503, { error: "Clé API manquante côté serveur. Configure MISTRAL_API_KEY dans .env." });
  }
  ...
  try {
    ...
    return sendJson(res, 400, { error: "URL invalide. Exemple : https://tonsite.com" });
  }

  if (!(await isPublicHost(target.hostname))) {
    return sendJson(res, 400, { error: "Cette adresse n'est pas accessible publiquement." });
  }
  ...
  try {
    ...
    return sendJson(res, 422, {
      error: "Impossible de charger cette page. Vérifie que le site est en ligne et accessible.",
    });
  }
  ...
  if (extract.text.length < 80) {
    return sendJson(res, 422, {
      error: "La page ne contient pas assez de contenu lisible pour un audit (site rendu 100 % en JavaScript ?).",
    });
  }

  try {
    const report = await runAudit(target.href, extract);
    return sendJson(res, 200, { url: target.href, finalUrl: page.finalUrl, report });
  } catch (err) {
    console.error("audit:", err.message);
    return sendJson(res, 502, { error: "L'analyse a échoué. Réessaie dans un instant." });
  }
}
