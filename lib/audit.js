...
/* Budget total (ms) sous le plafond de 60s de la fonction Vercel
   (vercel.json), avec une marge pour le parsing JSON final. Le fetch de la
   page a son propre timeout (15s) mais prend rarement plus de 1-2s en
   pratique — plutôt que de figer l'appel LLM à 40s fixes (et couper des
   réponses Mistral légitimes mais un peu lentes), le timeout du LLM
   s'adapte au temps réellement restant. */
const TOTAL_BUDGET_MS = 55000;
const MIN_LLM_TIMEOUT_MS = 20000;

function remainingBudget(startedAt) {
  return Math.max(MIN_LLM_TIMEOUT_MS, TOTAL_BUDGET_MS - (Date.now() - startedAt));
}
...
async function analyze(rawUrl) {
  ...
  try {
    const report = await runAudit(target.href, extract, cfg, remainingBudget(startedAt));
    if (report.rejet) {
    ...
    return {
      ...
      body: {
        ...
        ads: buildAdLinks(guessBrandName(extract, target.hostname), target.hostname),
        report,
      },
    };
  } catch (err) {
...
function guessBrandName(x, hostname) {
  const candidate = x.structured?.orgName || x.ogSiteName || titleBrand(x.title);
  return cleanBrand(candidate) || domainToName(hostname);
}

/* Premier segment du <title> avant un séparateur éditorial — les titres
   marketing complets ("Acme : la facturation 2× plus vite pour les
   agences") donnent des recherches publicitaires sans résultats. */
function titleBrand(title) {
  return (title || "").split(/\s+[-|·–—:]\s+|\s*[|•]\s*/)[0];
}

/* Un nom de recherche exploitable : court (les bibliothèques matchent mal
   les requêtes longues), sans ™/® ni ponctuation traînante. */
function cleanBrand(name) {
  const cleaned = (name || "").replace(/[™®©]/g, "").replace(/[.,;!?]+$/, "").trim();
  if (!cleaned || cleaned.length > 40) return "";
  const words = cleaned.split(/\s+/);
  return words.length > 4 ? "" : cleaned;
}
...
/* Liens directs vers les bibliothèques publicitaires publiques, préremplis
   ...
   aller. Formats calqués sur les URLs que les deux interfaces génèrent
   elles-mêmes (Meta : q= + search_type=keyword_unordered ; TikTok :
   adv_name= + query_type=1, trié par diffusion récente). En secours, les
   mêmes liens avec le nom dérivé du domaine, pour quand le nom détecté ne
   donne rien. */
function buildAdLinks(brand, hostname) {
  const links = (name) => {
    const q = encodeURIComponent(name);
    return {
      meta: `https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country=ALL&media_type=all&q=${q}&search_type=keyword_unordered`,
      tiktok: `https://library.tiktok.com/ads?region=all&query_type=1&sort_type=last_shown_date,desc&adv_name=${q}`,
    };
  };
  const main = links(brand);
  const domainName = domainToName(hostname);
  const alt = domainName && domainName.toLowerCase() !== brand.toLowerCase() ? links(domainName) : null;
  return {
    brand,
    meta_url: main.meta,
    tiktok_url: main.tiktok,
    alt_brand: alt ? domainName : null,
    meta_alt_url: alt ? alt.meta : null,
    tiktok_alt_url: alt ? alt.tiktok : null,
  };
}
...
async function runAudit(url, x, cfg, llmTimeoutMs = 40000) {
  const userPrompt = [
  ...
  /* Timeout dérivé du budget restant sous le plafond de 60s de la fonction
     Vercel (vercel.json) — voir remainingBudget(). Un appel LLM plus long
     ferait tuer la fonction par Vercel, qui répond alors en texte brut
     ("An error occurred…"), illisible côté front. Ici, un timeout devient
     une vraie erreur JSON propre ("L'analyse a échoué. Réessaie."). */
...
async function compare(rawMyUrl, competitor) {
  ...
  try {
    // Timeout adaptatif — même logique que runAudit, voir remainingBudget().
    const resp = await fetch(`${cfg.baseUrl}/v1/chat/completions`, {
      method: "POST",
      signal: AbortSignal.timeout(remainingBudget(startedAt)),
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${cfg.apiKey}`,
      },
