...
function extractContent(html, url) {
  ...
  return { url, title, metaDesc, h1, h2, h3, ctas: uniqueCtas, text, formCount, imgCount, hasPricing, hasTestimonials, tech, socials, ecom, metrics };
}
...
- Métriques business : MRR / ARR (revenu récurrent), churn, LTV, CAC, ratio LTV/CAC (sain à partir de 3), ARPU, panier moyen (AOV), taux de conversion (e-commerce : 1-3 % ; landing SaaS : 2-5 % ; au-delà = excellent), abandon de panier (~70 % en moyenne).
- Modèles de croissance : product-led growth (le produit vend : essai gratuit, freemium), sales-led (démo, vente accompagnée), content-led / SEO, paid (Meta, Google, TikTok — raisonne toujours en CAC vs marge), communauté, referral / bouche-à-oreille, partenariats / affiliation.
...
- Pricing : ancrage haut, offre decoy, mensuel vs annuel (l'annuel améliore le cash-flow et réduit le churn), essai gratuit vs garantie remboursement, freemium, bundles.
- Rétention et monétisation : email / CRM (Klaviyo, Mailchimp = travail de la LTV), upsell / cross-sell, seuil de livraison gratuite, abonnements, programmes de fidélité.

Adapte le raisonnement au modèle : un SaaS se juge en MRR, churn et activation ; un e-commerce en AOV, taux de conversion et coût d'acquisition ; un infoproduit en audience et taux de closing. Quand une mécanique du site sert une de ces métriques, dis laquelle et comment.
...
const COMPARE_SYSTEM_PROMPT = `Tu es consultant senior en growth marketing et conversion, expert des métriques business (MRR, ARR, churn, LTV, CAC, panier moyen, taux de conversion, funnel AARRR) et de la psychologie de conversion (preuve sociale, ancrage de prix, urgence, réduction de friction). Ton client vient de faire décoder le site d'un concurrent qui vend bien. Il te donne maintenant SON propre site. Ta mission : le comparer au concurrent et lui dire exactement quoi changer pour vendre autant. Quand un écart touche une métrique précise (conversion, AOV, churn, CAC…), nomme-la et explique l'impact.
...
Ton style : clair, concret, orienté business, format causal (élément → effet). Chaque comparaison cite des éléments réels des deux sites. Interdit : phrases génériques applicables à n'importe quel site. Français, tutoiement.
...
async function compare(rawMyUrl, competitor) {
  ...
  try {
    ...
    return { status: 422, body: { error: "Impossible de charger ton site. Vérifie qu'il est en ligne et accessible." } };
  }
  ...
  const userPrompt = [
    ...
    `H2 : ${x.h2.join(" | ") || "(aucun)"}`,
    `CTA / BOUTONS : ${x.ctas.join(" | ") || "(aucun détecté)"}`,
    `FORMULAIRES : ${x.formCount} — SECTION PRIX : ${x.hasPricing ? "oui" : "non"} — PREUVE SOCIALE : ${x.hasTestimonials ? "oui" : "non"}`,
    `INDICES TECHNIQUES : ${x.tech.join(", ") || "(aucun)"}`,
    `RÉSEAUX SOCIAUX LIÉS : ${x.socials.join(", ") || "(aucun)"}`,
    `SIGNAUX E-COMMERCE : ${x.ecom.join(", ") || "(aucun)"}`,
    ``,
