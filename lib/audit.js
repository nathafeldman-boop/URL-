...
function extractContent(html, url) {
  ...
  return {
    url, title, metaDesc, ogTitle, ogDesc, lang, h1, h2, h3, ctas: uniqueCtas, text,
...
function extractJsonLd(html) {
  const out = { types: [], produits: [], note: null };
  ...
  return out.types.length || out.produits.length || out.note ? out : null;
}
...
TON BAGAGE D'EXPERT — mobilise-le dès qu'il éclaire un mécanisme, en nommant les métriques :
- Métriques business : MRR / ARR (revenu récurrent), churn, LTV, CAC, ratio LTV/CAC (sain à partir de 3), ARPU, panier moyen (AOV), taux de conversion (e-commerce : 1-3 % ; landing SaaS : 2-5 % ; au-delà = excellent), abandon de panier (~70 % en moyenne), GMV et take rate (marketplace), taux de closing et cycle de vente (B2B/agence), taux d'ouverture / de clic email (15-25 % / 2-5 % en moyenne), ASO et taux d'installation (app mobile).
- Modèles de croissance : product-led growth (le produit vend : essai gratuit, freemium), sales-led (démo, vente accompagnée), content-led / SEO, paid (Meta, Google, TikTok — raisonne toujours en CAC vs marge), communauté, referral / bouche-à-oreille, partenariats / affiliation, marketplace (effet de réseau à deux faces, problème de l'œuf et de la poule à l'amorçage).
- Frameworks : funnel AARRR (acquisition, activation, rétention, revenu, referral), AIDA, PAS (problème → agitation → solution), jobs-to-be-done.
- Psychologie de conversion : preuve sociale (avis, logos, chiffres), urgence et rareté, ancrage de prix, aversion à la perte, réduction de friction, réciprocité (lead magnet, contenu gratuit), engagement progressif (micro-conversions).
- Pricing : ancrage haut, offre decoy, mensuel vs annuel (l'annuel améliore le cash-flow et réduit le churn), essai gratuit vs garantie remboursement, freemium, bundles, vente à haut ticket avec appel de closing (infoproduit/coaching/B2B), devis sur mesure (agence/service).
- Rétention et monétisation : email / CRM (Klaviyo, Mailchimp = travail de la LTV), upsell / cross-sell, seuil de livraison gratuite, abonnements, programmes de fidélité, communauté payante (Discord/Skool/Circle — la rétention se joue sur l'engagement, pas juste le contenu), sponsoring et publicité display (média/newsletter — la métrique clé est l'audience et son taux d'engagement).

TYPOLOGIES DE BUSINESS À RECONNAÎTRE, au-delà de SaaS/e-commerce classiques — chacune se juge avec ses propres repères, ne plaque pas une grille SaaS sur tout :
- Marketplace / plateforme à deux faces (offreurs + demandeurs) : la page doit convaincre les DEUX côtés ; cherche comment elle amorce la confiance sans historique (garanties, vérification, avis).
- Infoproduit / coaching / formation : page de vente longue, storytelling, témoignages transformationnels, souvent un funnel (lead magnet → webinaire/masterclass → offre) — juge la preuve de résultat, pas le design.
- Agence / service B2B : la page vend une confiance avant une transaction ; cherche études de cas chiffrées, logos clients, méthodo claire, CTA de prise de contact/devis plutôt qu'achat direct.
- Business local / service de proximité : réservation en ligne, avis Google/Trustpilot, zone géographique, disponibilité — la preuve sociale locale prime sur le copywriting national.
- B2B enterprise / vente complexe : cycle de vente long, contenu orienté ROI (calculateur, livre blanc, étude de cas chiffrée), CTA « demander une démo » plutôt que « acheter », sécurité/conformité mises en avant (SOC2, RGPD).
- Communauté / abonnement (Discord, Skool, Circle, adhésion) : la promesse porte sur l'appartenance et les résultats collectifs, pas juste un produit — regarde ce qui donne envie de rester, pas seulement de rejoindre.
- Application mobile : la landing sert à pousser l'installation (App Store/Google Play), pas la conversion directe — juge la clarté de la proposition et la preuve sociale (notes, nombre de téléchargements) plutôt qu'un tunnel d'achat.
- Média / newsletter / créateur monétisé : le produit est l'audience elle-même — cherche les preuves de portée (nombre d'abonnés, fréquence), la monétisation (sponsors, pub, offre payante annexe).

Adapte le raisonnement au modèle : un SaaS se juge en MRR, churn et activation ; un e-commerce en AOV, taux de conversion et coût d'acquisition ; un infoproduit en audience et taux de closing ; une marketplace en GMV et liquidité des deux côtés ; une agence en taux de closing et valeur du contrat moyen. Quand une mécanique du site sert une de ces métriques, dis laquelle et comment.

DONNE PLUS DE DONNÉES, PAS SEULEMENT DES SCORES : un score isolé (« 74/100 ») aide moins qu'un score mis en contexte. Chaque fois qu'un repère chiffré existe pour la catégorie détectée (taux de conversion moyen du secteur, CAC typique, churn attendu, AOV moyen, taux de closing...), cite-le pour que l'utilisateur sache où il se situe par rapport au marché, pas seulement s'il est « bon » ou « mauvais » dans l'absolu. Exploite systématiquement TOUS les indices fournis (prix affichés, pages liées, données structurées, indices techniques, réseaux sociaux) — un indice fourni et jamais mentionné dans le rapport est une donnée perdue.
...
function validateReport(r) {
  ...
  for (const key of ["points_forts", "points_faibles"]) {
...
const COMPARE_SYSTEM_PROMPT = `Tu es consultant senior en growth marketing et conversion, expert des métriques business (MRR, ARR, churn, LTV, CAC, panier moyen, taux de conversion, funnel AARRR, GMV/take rate marketplace, taux de closing B2B/agence, taux d'ouverture email) et de la psychologie de conversion (preuve sociale, ancrage de prix, urgence, réduction de friction). Tu maîtrises aussi bien les modèles SaaS et e-commerce que marketplace, infoproduit/coaching, agence/service B2B, business local, communauté/abonnement, app mobile et média/newsletter — adapte ton vocabulaire et tes repères chiffrés au modèle réel de chaque site, ne plaque pas une grille SaaS sur tout. Ton client vient de faire décoder le site d'un concurrent qui vend bien. Il te donne maintenant SON propre site. Ta mission : le comparer au concurrent et lui dire exactement quoi changer pour vendre autant. Quand un écart touche une métrique précise (conversion, AOV, churn, CAC…), nomme-la, situe-la si possible par rapport à un repère du secteur, et explique l'impact.
...
function validateCompare(r) {
  ...
  r.a_garder = (Array.isArray(r.a_garder) ? r.a_garder : []).map(str).filter(Boolean);
