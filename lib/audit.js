...
async function runAudit(url, x, cfg) {
  ...
  /* 40s max : la fonction Vercel est plafonnée à 60s (vercel.json). Avec le
     fetch de la page (≤15s) en amont, un appel LLM plus long ferait tuer la
     fonction par Vercel — qui répond alors en texte brut ("An error
     occurred…"), illisible côté front. Ici, un timeout devient une vraie
     erreur JSON propre ("L'analyse a échoué. Réessaie."). */
...
async function compare(rawMyUrl, competitor) {
  ...
  try {
    // 40s max — même budget que runAudit, voir le commentaire là-bas.
    const resp = await fetch(`${cfg.baseUrl}/v1/chat/completions`, {
      method: "POST",
      signal: AbortSignal.timeout(40000),
      headers: {
